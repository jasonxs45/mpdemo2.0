...
import { _detail, _submit, _mydetail } from '../../api/coupon'
    ...
    getDetail() {
      ...
      _detail(id)
        .then(res => {
          ...
          if (code === 0) {
            data.ExchangeEndTime = formatDate(new Date(data.ExchangeEndTime), 'yyyy-MM-dd')
            this.set({
              detail: data,
              words: data.CouponInstruction
            })
          } else {
            wx.showModal({
              title: '对不起',
    ...
    getMyDetail() {
      ...
      _mydetail({ ID, UnionID })
        .then(res => {
          const { code, msg, data } = res.data
          if (code === 0) {
            if (data && data.UserCouponTradeLog) {
              this.set({
                myInfo: data.UserCouponTradeLog
              })
                .then(() => {
                  const { myInfo } = this.data
                  if (myInfo && myInfo.State === '已核销') {
                    this.timer && clearInterval(this.timer)
                  }
                })
            }
